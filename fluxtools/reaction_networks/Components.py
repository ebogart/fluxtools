...
class Event:
    ...
    def parseTrigger(self, trigger):
        ...
        if expr_manip.extract_vars(trigger) == sets.Set(['time']):
            self.timeTriggered = True
            ast = expr_manip.AST.strip_parse(trigger)
            firstArg = expr_manip.AST.ast2str(ast.args[0])
            secondArg = expr_manip.AST.ast2str(ast.args[1])
