...
import logging
import os
logger = logging.getLogger('expr_manip.Differentiation')
...
def load_derivs(filename):
    ...
    """
    # The global statement is necessary so that we can 
    # assign to the module-level variable __deriv_saved 
    ...
    try:
        ...
        logger.debug('Loaded cached derivatives from %s.' % filename)
        return
    except:
...
def save_derivs(filename):
    """ Update the derivative file with new derivative information. """
...
def diff_expr(expr, wrt):
    ...
    saved_result = __deriv_saved.get(expr, {}).get(wrt, None) 
    if saved_result is not None:
        logger.debug('Found saved result %s.' % saved_result)
        return saved_result
    ...
    __deriv_saved.setdefault(expr, {})[wrt] = deriv
